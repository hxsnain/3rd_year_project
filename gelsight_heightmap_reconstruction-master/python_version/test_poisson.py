...
def show_depth(depth, scale_factor):
    # create a new figure and add an Axes3D object
    fig = plt.figure()
    ax = Axes3D(fig)

    # create X,Y grid and plot the depth map as a surface
    X,Y = np.meshgrid(range(depth.shape[1]), range(depth.shape[0]))
    ax.plot_surface(X, Y, depth/scale_factor, cmap='jet')
    ...
    ax.set_ylim(0, depth.shape[0])
    ax.set_zlim(0, np.max(depth)/scale_factor)
...
if __name__ == '__main__': 
    ...
    cali = calibration()
    pad = 20
    ref_img = cv2.imread('C:/Users/hasna/OneDrive/Documents/Year_3/3rd_year_project/gelsight_heightmap_reconstruction-master/python_version/test_data/ref.jpg')
    ref_img = ref_img[x_index, y_index, :]
    test_img = cv2.imread('C:/Users/hasna/OneDrive/Documents/Year_3/3rd_year_project/gelsight_heightmap_reconstruction-master/python_version/test_data/sample_20.jpg')
